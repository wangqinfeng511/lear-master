### modbus:

//未校验，1位起始位 8位数据位 1位奇偶校验位 1位停止位 4位crc校验位

//项目采用的是：

```c++
QSerialPort *com_port=new QSerialPort;
//     qDebug()<<"端口："<<com;
     com_port->setPortName(com);
     com_port->setBaudRate(QSerialPort::Baud9600);
     com_port->setDataBits(QSerialPort::Data8);
     com_port->setStopBits(QSerialPort::OneStop);
     com_port->setParity(QSerialPort::NoParity);
     com_port->setFlowControl(QSerialPort::NoFlowControl);
     com_port->open(QSerialPort::ReadWrite);
//     qDebug()<<action<<com_port->isOpen();
     if(!com_port->isOpen()){
         return false;
     }
     //写入数据
     com_port->write(requset);
     //阻塞延时等待数据
     com_port->waitForReadyRead(-1);
```



## CRC字典

```c++
const unsigned char aucCRCHi[] = {
    };
const unsigned char aucCRCLo[] = {
    };

```

```c++
//差出数据的crc 码
QByteArray return_crc(unsigned char *datas,int data_len){
    unsigned char Hi=0xFF;
    unsigned char Li=0xFF;
//char 列表递减
    while (data_len--) {
        //递增取出unsigned char
        unsigned char tmp_char=*datas++;
        unsigned int d_t=tmp_char;
        //<按位于,取出字典索引
        unsigned int index=  Hi^ tmp_char;
//        qDebug()<<"index"<<index;
        Hi=Li^aucCRCHi[index];
        Li=aucCRCLo[index];
    }
    QByteArray crc_value;
    crc_value.append(Hi);
    crc_value.append(Li);
    return crc_value; //返回校验码。
}
//数据+crc码 
```